...
<title>Teacher Login — EduSystem</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet">
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{min-height:100vh;font-family:'Inter',sans-serif;background:linear-gradient(135deg,#1e1b4b 0%,#3730a3 45%,#0ea5e9 100%);display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;}
...
.card{background:#fff;border-radius:24px;padding:48px 44px;width:100%;max-width:440px;box-shadow:0 32px 80px rgba(0,0,0,.3);position:relative;z-index:1;}
.brand{text-align:center;margin-bottom:32px;}
.brand-icon{width:72px;height:72px;background:linear-gradient(135deg,#3730a3,#0ea5e9);border-radius:20px;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:0 8px 24px rgba(55,48,163,.4);}
...
.input-wrap input:focus{border-color:#3730a3;box-shadow:0 0 0 4px rgba(55,48,163,.1);}
...
.btn-login{width:100%;padding:14px;background:linear-gradient(135deg,#3730a3,#0ea5e9);color:#fff;border:none;border-radius:12px;font-family:'Inter',sans-serif;font-size:1rem;font-weight:700;cursor:pointer;transition:opacity .2s,transform .1s;margin-top:8px;}
...
.student-link{display:block;text-align:center;color:#3730a3;font-size:.875rem;font-weight:600;text-decoration:none;transition:color .2s;}
.student-link:hover{color:#0ea5e9;}
</style>
</head>
<body>
<div class="card">
  ...
  <form action="{{ route('teacher.login.post') }}" method="POST">
    ...
    <div class="form-group">
      ...
      <div class="input-wrap">
        ...
        <input type="email" name="email" value="{{ old('email') }}" placeholder="teacher@school.lk" required autofocus>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="input-wrap">
        ...
        <button type="button" class="toggle-pw" onclick="togglePw()"><i class="bi bi-eye" id="eyeIcon"></i></button>
      </div>
    </div>
